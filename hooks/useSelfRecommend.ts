...
export function useSelfRecommend() {
  ...
  const fetchPlaces = async (latitude: number, longitude: number, page: number = 1) => {
    ...
    try {
      ...
      setPage(page)
      console.log('Successfully fetched self-recommended places:', response.data.data.content.length, 'items')
    } catch (error) {
      console.error('Failed to fetch self-recommended places:', error)
      setError('맞춤 추천 카페를 불러오는데 실패했습니다.')
    } finally {
      if (page === 1) {
        setIsLoading(false)
      } else {
